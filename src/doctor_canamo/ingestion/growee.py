...
class GroweeConnector(DataConnector):
    ...
    def fetch_metrics(self) -> List[Metric]:
        if not self.base_url or not self.token:
            self.logger.debug("Sin credenciales de Growee: devolviendo métricas sintéticas")
            return self._synthetic_metrics()

        headers = {"Authorization": f"Bearer {self.token}"}
        self.logger.debug("Llamando Growee en %s", self.base_url)
        ...
        self.logger.info("Recibidas %d métricas de Growee", len(metrics))
        return metrics
