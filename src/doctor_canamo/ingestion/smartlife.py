...
class SmartLifeConnector(DataConnector):
    ...
    def fetch_metrics(self) -> List[Metric]:
        if not self.api_url or not self.access_token:
            self.logger.debug("Sin credenciales de SmartLife: devolviendo métricas sintéticas")
            return self._synthetic_metrics()

        headers = {"Authorization": f"Bearer {self.access_token}"}
        self.logger.debug("Llamando SmartLife en %s", self.api_url)
        ...
        self.logger.info("Recibidas %d métricas de SmartLife", len(metrics))
        return metrics
